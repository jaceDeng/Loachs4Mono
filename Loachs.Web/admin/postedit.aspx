<%@ Page Language="C#" MasterPageFile="admin.master" AutoEventWireup="true" ValidateRequest="false"
    Inherits="Loachs.Web.admin_postedit" Title="无标题页" CodeBehind="postedit.aspx.cs" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <%=ResponseMessage %>
    <p>
        <label class="label" for="<%=txtTitle.ClientID %>">
            标题:</label>
        <asp:TextBox ID="txtTitle" Width="95%" runat="server" CssClass="text"></asp:TextBox>
    </p>
    <p>
        <label class="label" for="<%=txtContents.ClientID %>">
            正文:<span class="small gray"> </span><a href="upfilebyeditor.aspx?keepThis=true&getfile=1&TB_iframe=true&height=480&width=780"
                title="插入图片/文件" class="thickbox" target="_blank">插入图片/文件</a>
        </label>
        <asp:TextBox ID="txtContents" runat="server" Height="400" TextMode="MultiLine" Width="95%"></asp:TextBox>
        <label class="label gray">
            提示：Enter产生&lt;p&gt;(换段), Shift+Enter产生&lt;br/&gt;(换行)</label>
    </p>
    <script type="text/javascript">
...
function createSummary(type) {
    ...
    <p>
        <label class="label" for="<%=txtSummary.ClientID %>">
            摘要: <a href="javascript:void(0);" onclick="createSummary('full');">从正文提取全部</a>/<a
                href="javascript:void(0);" onclick="createSummary('part');">部分</a></label>
    ...
    <p>
        <label class="label" for="<%=txtTags.ClientID %>">
            标签:<span class="gray small"> (多个标签用逗号隔开[选填])</span></label>
        <asp:TextBox ID="txtTags" runat="server" Width="49%" CssClass="text"></asp:TextBox>
        <a href="###" onclick="showTag();">查看常用标签↓</a>
        <div id="taglist" style="border: 2px solid #ccc; display: none; line-height: 135%;
            padding: 3px;">
            <%
            ...
            <a href="###" style="padding-left: 3px;">
                <%=tag.Name %></a><span class="gray small">(<%=tag.Count %>)</span>
            <%} %>
            <%if (tagList.Count == 0)
              { %>
            暂无
    ...
    <p>
        <label class="label" for="<%=txtCustomUrl.ClientID %>">
            别名:<span class="gray small"> (字母,数字,中文,连字符[选填])</span></label>
        <asp:TextBox ID="txtCustomUrl" runat="server" Width="49%" CssClass="text"></asp:TextBox>
    </p>
    <p>
        <label class="label" for="<%=ddlUrlType.ClientID %>">
            Url地址格式:</label>
        <asp:DropDownList ID="ddlUrlType" runat="server" Width="50%" CssClass="text">
        </asp:DropDownList>
    </p>
    <p>
        <label class="label" for="<%=ddlTemplate.ClientID %>">
            文章模板:<span class="gray small"> (位于<%=ConfigHelper.SitePath %>themes/<%=setting.Theme %>/template/下,当然,您也可以自己制作,文件名必须以"post"开头)</span></label>
        <asp:DropDownList ID="ddlTemplate" runat="server" Width="50%">
        </asp:DropDownList>
    </p>
    <p>
        <label class="label" for="<%=ddlCategory.ClientID %>">
            分类:</label>
        <asp:DropDownList ID="ddlCategory" runat="server" Width="50%">
        </asp:DropDownList>
    </p>
    <p>
    ...
    <p>
        <asp:Button ID="btnEdit" runat="server" Text="添加" OnClick="btnEdit_Click" CssClass="button" /></p>
</asp:Content>
